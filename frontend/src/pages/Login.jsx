...
const Login = () => {
    ...
    const handleLogin = async (event) => {
        ...
        if (!username || !password) {
            toast.warn("Please fill in both fields"); // ✅ Toast instead of alert
            setLoading(false);
            return;
        }
        ...
        try {
            ...
            navigator.geolocation.getCurrentPosition(
                ...
                (error) => {
                    ...
                    toast.info("Login successful (location not shared)."); // ✅ Toast
                    navigate('/app');
                }
            );
        } catch (error) {
            console.error("Login error:", error.response?.data || error.message);
            toast.error(error.response?.data?.message || "An error occurred during login."); // ✅ Toast
        } finally {
            setLoading(false);
        }
    };
